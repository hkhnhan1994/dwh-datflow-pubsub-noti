...
def getTopic(
    ...
    project: str,
    database_info:dict,
    topic_path= 'stream_name',
    aditional_prefix:str = None):
    obj = data.decode("utf-8")
    obj = json.loads(obj) 
    get_topic=obj.get(topic_path)
    '''Get topic name from each record in GCS.'''
    _exist_topic = None
    if get_topic:
        incomming_topic_name=((get_topic.split('/'))[-1]).replace('-','_')
        incomming_topic_name = "{}_{}".format(aditional_prefix,incomming_topic_name)
        publisher = pubsub_v1.PublisherClient()
        incomming_topic_path=publisher.topic_path(project, incomming_topic_name)
        print(" topic: {} in project {}".format(project, incomming_topic_name))
        print(database_info)
        if database_info['TOPIC'] == incomming_topic_name:
            _exist_topic=publisher.get_topic(request={"topic": incomming_topic_path})
    return incomming_topic_path, _exist_topic
def getSub(project: str, Subscriber_name: str):
        ...
        try:
            _exist_Subscriber=Subscriber.get_subscription(request={"subscription": subscription_path})
            # print(f"sub {Subscriber_name} found: {_exist_Subscriber}")
            return Subscriber_name, _exist_Subscriber
        except Exception as e:
             logging.debug(e)
             return Subscriber_name, None
